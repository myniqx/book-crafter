...
function validatePath(filePath: string, workspaceRoot?: string): string {
  ...
  if (normalized.includes('..')) {
    throw createIPCError('Invalid path: directory traversal not allowed', 'INVALID_PATH')
  }
  ...
  if (workspaceRoot) {
    ...
    if (!resolvedPath.startsWith(resolvedRoot)) {
      throw createIPCError('Path outside workspace root', 'PERMISSION_DENIED')
    }
  }
  ...
  ipcMain.handle(
    ...
    async (_event: IpcMainInvokeEvent, dirPath: string, options?: ReadDirOptions) => {
      try {
        ...
        if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
          throw createIPCError('Directory not found', 'FILE_NOT_FOUND', { path: dirPath })
        }
