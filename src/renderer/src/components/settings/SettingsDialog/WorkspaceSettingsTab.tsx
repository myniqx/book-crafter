    ...
    <div className="space-y-6">
        ...
        {workspacePreferences.autoBackup && (
          <>
            <FormField
              ...
              label={<>Backup Interval: <span className="font-normal text-muted-foreground">{workspacePreferences.backupInterval} minutes</span></>}
            >
              <div className="py-1">
                <Slider
                  id="backup-interval"
                  ...
                  onValueChange={([value]) => updateWorkspacePreferences({ backupInterval: value })}
                />
              </div>
            </FormField>
            <FormField
              htmlFor="max-backups"
              label={<>Maximum Backups: <span className="font-normal text-muted-foreground">{workspacePreferences.maxBackups}</span></>}
              hint="Older backups will be automatically deleted"
            >
              <div className="py-1">
                <Slider
                  id="max-backups"
            ...
            <FormField
              htmlFor="backup-path"
              label="Backup Path"
              hint="Leave empty to use default workspace .backups/ folder"
            >
      ...
      <div className="space-y-4">
        ...
        {workspacePreferences.watchExternalChanges && (
          <FormField
            htmlFor="reload-behavior"
            label="Reload Behavior"
            hint="How to handle files changed by external programs"
          >
            <Select
      ...
      <div className="space-y-4">
        ...
        <FormField
          ...
          label={<>Maximum File Size: <span className="font-normal text-muted-foreground">{workspacePreferences.maxFileSize}MB</span></>}
          hint="Files larger than this will not be opened in the editor"
        >
          <div className="py-1">
            <Slider
              id="max-file-size"
              ...
              onValueChange={([value]) => updateWorkspacePreferences({ maxFileSize: value })}
            />
          </div>
        </FormField>
      </div>
